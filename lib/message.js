...
const getSigninUrl = (openUserId) => {
  return isSonyIntegration ?
    `${config.excelsiorServer}/n/sony/members/sign_in?user=${openUserId}` :
    `${config.excelsiorServer}/n/nike/members/sign_in?user=${openUserId}`;
};
...
const getSubscribeMessage = (openUserId) => {
  return `您好！为了保证您能接收到系统的最新消息通知，请新进行<a href="${getSigninUrl(openUserId)}">身份验证</a>,即可享受一下服务：
